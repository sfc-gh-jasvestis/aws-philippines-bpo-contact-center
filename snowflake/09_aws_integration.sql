...
CREATE OR REPLACE EXTERNAL ACCESS INTEGRATION aws_philippines_bpo_contact_center_BEDROCK_EAI
  ALLOWED_NETWORK_RULES = (CONTACT_CENTER_AI.APP.BEDROCK_NETWORK_RULE)
  ALLOWED_AUTHENTICATION_SECRETS = (CONTACT_CENTER_AI.APP.AWS_BEDROCK_SECRET)
  ENABLED = TRUE
  COMMENT = 'Bedrock access for Contact Center Intelligence & CSAT Optimization';
...
CREATE OR REPLACE FUNCTION APP.BEDROCK_GENERATE(prompt VARCHAR)
  ...
  EXTERNAL_ACCESS_INTEGRATIONS = (aws_philippines_bpo_contact_center_BEDROCK_EAI)
  SECRETS = ('aws_creds' = CONTACT_CENTER_AI.APP.AWS_BEDROCK_SECRET)
AS $$
