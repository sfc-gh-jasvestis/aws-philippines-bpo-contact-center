...
export default function HomePage() {
  ...
  const executiveCockpit = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard title="Calls Handled (MTD)" value={kpiVal('Calls Handled (MTD)', '2.4M')} status="neutral" />
        <KPICard title="Avg Handle Time" value={kpiVal('Avg Handle Time', '4:32')} status="warning" />
        <KPICard title="CSAT Score" value={kpiVal('CSAT Score', '4.2/5')} status="neutral" />
        <KPICard title="Agents Active" value={kpiVal('Agents Active', '12,400')} status="neutral" />
      </div>
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="lg:col-span-1">
          <GeoMap
            country="philippines"
            labels={{ entity: 'Queues', event: 'Interactions', alert: 'Escalations' }}
      ...
      <div className="grid grid-cols-1 gap-4 grid-cols-1">
        <Chart
          ...
          type="line"
          xKey="period"
          yKeys={[{ key: 'value', name: 'Calls (K)' }]}
          title="Call Volume (Hourly)"
        />
        <Chart
      ...
      <DataTable
        columns={[
          ...
          { key: 'm1', header: 'AHT (min)' },
          { key: 'm2', header: 'Csat Score' },
          { key: 'm3', header: 'First Call Resolution' },
          { key: 'events', header: 'Interactions' },
          { key: 'alerts', header: 'Escalations' },
        ]}
        data={data?.entities || []}
  ...
  const domainTab1 = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <KPICard title="First Call Resolution" value={kpiVal('First Call Resolution', '78%')} />
        <KPICard title="Occupancy Rate" value={kpiVal('Occupancy Rate', '84%')} />
        <KPICard title="Agent Attrition" value={kpiVal('Agent Attrition', '4.2%/mo')} />
      </div>
      <Chart
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Chart
          ...
          title="Staffing vs Demand Forecast"
        />
        <ActionMemo
          persona={{ name: 'Carlo Mendoza', role: 'Contact Center Director' }}
          context={{}}
          onGenerate={async () => {
            const memos = [
              {
                ...
                actions: ['Add 200 FTE for Q4 retail client peak season', 'Deploy AI chatbot to deflect password reset calls (18% of volume)', 'Investigate AHT increase in billing queue (agent knowledge gap)'],
              },
              {
    ...
    <div className="h-[600px]">
      <AskAI
        ...
        sampleQuestions={[
          ...
          'Show agent productivity by shift schedule',
          'What is the optimal staffing level for holiday season?',
        ]}
        mode="sql"
  ...
  const tabs = [
    ...
    { id: 'domain-1', label: 'Agent Performance', icon: '📈', content: domainTab1 },
    { id: 'domain-2', label: 'Workforce', icon: '⚡', content: domainTab2 },
