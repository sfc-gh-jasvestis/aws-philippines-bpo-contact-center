...
export default function HomePage() {
  ...
  const domainTab1 = (
    <div className="space-y-6">
      ...
      <Chart
        ...
        title="Contact Center Performance Trend"
        height={400}
      />
    </div>
  );
  ...
  const domainTab2 = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        />
        <ActionMemo
          persona={{ name: 'Philippines Operations Lead', role: 'Director of Contact Center' }}
          context={{}}
          onGenerate={async () => ({
            subject: 'Action Required',
            body: 'AI-generated recommendation based on current data patterns and predicted trends.',
            urgency: 'HIGH',
            actions: ['Review top sla breaches findings', 'Optimize site allocation', 'Prepare quarterly contact center report'],
          })}
        />
      </div>
    </div>
  ...
  const tabs = [
    ...
    { id: 'domain-1', label: 'Contact Center Analytics', icon: '📈', content: domainTab1 },
